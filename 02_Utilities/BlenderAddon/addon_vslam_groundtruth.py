...
class VSLAMExportCamera(Operator):
    ...
    def execute(self, context):
        ...
        filepath_ideal = scene.vslam_output_directory + camera.name + "_ideal.csv"
        filepath_noisy = scene.vslam_output_directory + camera.name + "_noisy.csv"
        
        if(scene.vslam_bool_export_ideal):
            FileIdealCamera = open(filepath_ideal, 'w', encoding='utf-8')
        if(scene.vslam_bool_export_noisy):
            FileNoisyCamera = open(filepath_noisy, 'w', encoding='utf-8')
        
        if(scene.objects.get(scene.vslam_altered_camera) is not None):
            # deselect all
            bpy.ops.object.select_all(action='DESELECT')
            # select altered camera object
            scene.objects[scene.vslam_altered_camera].select = True
            # remove it
            bpy.ops.object.delete()
        
        scene.frame_set(scene.frame_start)
        bpy.ops.object.select_all(action='DESELECT')
        scene.objects[scene.vslam_camera].select = True
        bpy.ops.object.duplicate(linked=False, mode="TRANSLATION")
        print(scene.objects.active)
        altered_camera = scene.objects.active
        ...
        for frame in range(scene.frame_start, scene.frame_end + 1):
            ...
            if(scene.vslam_bool_export_noisy):
                if(scene.vslam_bool_export_timestamp):
                    FileNoisyCamera.write("%d,%f,%f,%f,%f,%f,%f,%f\n" % (timestamp, dso_trans.x, dso_trans.y, dso_trans.z, dso_quat.w, dso_quat.x, dso_quat.y, dso_quat.z))
                else:
                    FileNoisyCamera.write("%f,%f,%f,%f,%f,%f,%f\n" % (dso_trans.x, dso_trans.y, dso_trans.z, dso_quat.w, dso_quat.x, dso_quat.y, dso_quat.z))
        ...
        if(scene.vslam_bool_export_ideal):
            FileIdealCamera.close()
        if(scene.vslam_bool_export_noisy):
            FileNoisyCamera.close()
...
class VSLAMToolPanel(Panel):
    ...
    bl_space_type = "VIEW_3D" # show in 3D View
    bl_region_type = "TOOLS"  # more specifically: tool panel
    ...
    def draw(self, context):
        ...
        row = commands.row()
        row.operator("vslam.import_dso", text="Import DSO (\"camera_dso.csv\")", icon="MANIPUL")
        row = commands.row()

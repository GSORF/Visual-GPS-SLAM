...
def parseLDSO(filepath, obj, scene):
    with open(filepath) as poses:  
        ...
        while line:
            ...
            if len(line) >= 8:
                # Line contains:
                # 0: Timestamp in milliseconds (float), i.e. 1464887310.9831
                # 1: Translation along x in meters (float), i.e. -0.00957385779483108 
                # 2: Translation along y in meters (float), i.e. -0.00838783460396716
                # 3: Translation along z in meters (float), i.e.  0.0059669411706191
                # 4: Orientation along x (float), i.e. -2.39274759246283e-05
                # 5: Orientation along y (float), i.e. 6.84733407121811e-05
                # 6: Orientation along z (float), i.e. -3.59334788291032e-05
                # 7: Orientation along w (float), i.e. 0.999999996723831
                # print(line)
                pass
            else:
                continue
            ...
            frame = int(timestamp-first_timestamp)
            location = [ float(line[1]), float(line[2]), float(line[3]) ]
            orientation = [ float(line[7]), float(line[4]), float(line[5]), float(line[6]) ]
            
            # print("LDSO-Importer: Now setting frame =", count)
...
class VSLAMImportLDSO(Operator):
    '''
    This operator imports camera poses from the LDSO.
    '''
    
    bl_idname = "vslam.import_ldso"
    bl_label = "Import LDSO TXT File (results.txt, results.txt.noloop)"
    
    
    def execute(self, context):
        
        scene = context.scene
        camera_loop = "results.txt"
        camera_noloop = "results.txt.noloop"
        filepath_loop = scene.vslam_output_directory + camera_loop
        filepath_noloop = scene.vslam_output_directory + camera_noloop
        ldso_camera_loop = "Camera_LDSO_loop"
        ldso_camera_noloop = "Camera_LDSO_noloop"
        camera_object_loop = scene.objects.get(ldso_camera_loop)
        camera_object_noloop = scene.objects.get(ldso_camera_noloop)
        
        if(camera_object_loop is None):
            bpy.ops.object.camera_add(location=(0.0, 0.0, 0.0), rotation=(0.0, 0.0, 0.0))
            camera_object_loop = context.selected_objects[0]
            
        if(camera_object_noloop is None):
            bpy.ops.object.camera_add(location=(0.0, 0.0, 0.0), rotation=(0.0, 0.0, 0.0))
            camera_object_noloop = context.selected_objects[0]
            
        camera_object_loop.name = ldso_camera_loop
        camera_object_noloop.name = ldso_camera_noloop
        
        scene.render.fps = 1000
        parseLDSO(filepath_loop, camera_object_loop, scene)
        parseLDSO(filepath_noloop, camera_object_noloop, scene)
...
class VSLAMToolPanel(Panel):
    ...
    bl_idname = "SCENE_PT_VSLAMToolPanel"
    ...
    def draw(self, context):
        ...
        row = commands.row()
        row.operator("vslam.import_ldso", text="Import LDSO (\"results.txt\",\"results.txt.noloop\")", icon="EMPTY_DATA")
        row = commands.row()
...
def register():
    ...
    bpy.utils.register_class(VSLAMImportLDSO)
...
def unregister():
    ...
    bpy.utils.unregister_class(VSLAMImportLDSO)
